...
const _vc = new VirtualConsole();
_vc.on("jsdomError", (e) => _domErrors.push((e && (e.detail && e.detail.stack || e.detail || e.message)) || String(e)));
const dom = new JSDOM(htmlNoScript, { runScripts: "dangerously", pretendToBeVisual: true, url: "http://localhost/", virtualConsole: _vc });
const { window } = dom;
window.confirm = () => true;
window.alert = () => {};
window.prompt = () => "テスト企業";
...
test("詳細から複製すると新規契約モーダルが期間更新済みで開く", () => {
  ...
  const dupBtn = [...doc.querySelectorAll("#modalFoot .btn")].find((b) => b.textContent.includes("複製"));
...
test("ダッシュボードのKPIにホバー詳細(has-tip)が付与される", () => {
  doc.querySelector('.nav-item[data-view="dashboard"]').click();
  assert.ok(doc.querySelectorAll(".kpi.has-tip").length >= 6, "全体/有効のKPIにツールチップ");
  assert.ok(doc.querySelector(".stack-seg.has-tip"), "ステータスバーにツールチップ");
});
...
test("契約一覧の CSV出力 ボタンが絞り込み結果を書き出す", () => {
  ...
  search.value = "";
  search.dispatchEvent(new window.Event("input", { bubbles: true }));
  const btn = [...doc.querySelectorAll(".toolbar button")].find((b) => b.textContent.includes("CSV書き出し"));
  assert.ok(btn, "CSV書き出しボタンがある");
  let clicked = false;
...
test("契約一覧のCSV書き出しでエラーが起きない", () => {
  ...
  const btn = [...doc.querySelectorAll(".toolbar button")].find((b) => b.textContent.includes("CSV書き出し"));
  assert.ok(btn, "契約一覧にCSV書き出しボタンがある");
  btn.click();
  doc.createElement = origCreate;
  assert.ok(clicked, "ダウンロードがトリガーされる");
});
